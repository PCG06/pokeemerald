...
bool32 DoesSpeciesHaveSet(enum Species species)
{
    return (gPokemonSets[species].moves[0] > MOVE_POUND && gPokemonSets[species].ability > ABILITY_NONE) || species == SPECIES_EGG;
}

u32 GetSpeciesAbilityNum(enum Species species, enum Ability ability)
{
...
u32 CanMonLearnMove(enum Species species, enum Move move)
{
    const struct LevelUpMove *learnset = GetSpeciesLevelUpLearnset(species);
