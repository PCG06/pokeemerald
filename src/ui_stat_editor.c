...
static bool8 CheckIfStatCantDecrease(void);
...
void Task_OpenStatEditorFromLMenu(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        CleanupOverworldWindowsAndTilemaps();
        StatEditor_Init(CB2_ReturnToFieldWithOpenLMenu);
        DestroyTask(taskId);
    }
}
...
static void PrintMonStats()
{
    ...
    StringCopy(gStringVar2, gAbilitiesInfo[GetAbilityBySpecies(sStatEditorDataPtr->speciesID, GetMonData(ReturnPartyMon(), MON_DATA_ABILITY_NUM), FALSE)].name);
...
// if sStatEditorDataPtr->editingStat == 255 if youre editing EVs, 31 if youre editing IVs OR if youre editing EVs and youre at the total of 510
...
static bool8 CheckIfStatCantIncrease(void) 
{
    ...
    if (sStatEditorDataPtr->selector_x == EDITING_IVS && sStatEditorDataPtr->editingStat == IV_MAX_SINGLE_STAT)
        return TRUE;
...
{
    u16 iterator = 0;
    if((input <= EDIT_INPUT_MAX_INCREASE_STATE) && CheckIfStatCantIncrease())
    {
        StartSpriteAnim(&gSprites[sStatEditorDataPtr->selectorSpriteId], 2);
        return;
    }

    if((input >= EDIT_INPUT_DECREASE_STATE) && ((sStatEditorDataPtr->editingStat == STAT_MINIMUM) || CheckIfStatCantDecrease()))
    {
        StartSpriteAnim(&gSprites[sStatEditorDataPtr->selectorSpriteId], 1);
        return;
    }
    ...
    switch(input)
    {
        case EDIT_INPUT_DECREASE_STATE:
            ...
            {
                if(!(sStatEditorDataPtr->editingStat == STAT_MINIMUM) || !CheckIfStatCantDecrease())
