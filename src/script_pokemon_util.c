...
{
    // const data
    enum Item item = gPokemonSets[species].item;
    u32 nature = gPokemonSets[species].nature;
    u32 abilityNum = GetSpeciesAbilityNum(species, gPokemonSets[species].ability);
    enum Type teraType = gPokemonSets[species].teraType;
    ...
    for (u32 i = 0; i < MAX_MON_MOVES; i++)
        moves[i] = gPokemonSets[species].moves[i];

    // default data
    enum PokeBall ball = BALL_POKE;
    u32 gender = MON_GENDER_RANDOM;
    enum ShinyMode shinyMode = SHINY_MODE_RANDOM;
    bool32 gmaxFactor = FALSE;
    u32 dmaxLevel = 0;

    return ScriptGiveMonParameterized(0, PARTY_SIZE, species, MAX_LEVEL, item, ball, nature, abilityNum, gender, evs, ivs, moves, shinyMode, gmaxFactor, teraType, dmaxLevel);
}
