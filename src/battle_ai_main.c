...
void SetBattlerAiData(u32 battler, struct AiLogicData *aiData)
{
    ...
    aiData->speedStats[battler] = GetBattlerTotalSpeedStatArgs(battler, ability, holdEffect, 0);
}
...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    u32 aiMoveKillsPlayer = CanTargetMoveFaintAi(move, battlerAtk, battlerDef, 1);
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        if (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_JUSTIFIED:
                if (moveType == TYPE_DARK && !IS_MOVE_STATUS(move) && !aiMoveKillsPlayer)
                    RETURN_SCORE_MINUS(10);
                break;
    ...
    // other
        ...
        case EFFECT_CLANGOROUS_SOUL:
            if (gBattleMons[battlerAtk].hp <= gBattleMons[battlerAtk].maxHP / 3)
                ADJUST_SCORE(-10);
            else if (HasBattlerSideAbility(battlerDef, ABILITY_UNAWARE, aiData))
                ADJUST_SCORE(-10);
            break;
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_WATER_COMPACTION:
                if (moveType == TYPE_WATER && GetNoOfHitsToKOBattler(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex, AI_ATTACKING_PARTNER, CONSIDER_ENDURE) >= 4)
                {
...
static bool32 ShouldCompareMove(u32 battlerAtk, u32 battlerDef, u32 moveIndex, u16 move)
{
    ...
    if (gMovesInfo[move].power == 0)
        return FALSE;
    if (GetNoOfHitsToKOBattler(battlerAtk, battlerDef, moveIndex, AI_ATTACKING_ON_FIELD, DONT_CONSIDER_ENDURE) == 0)
        return FALSE;
...
static void AI_CompareDamagingMoves(u32 battlerAtk, u32 battlerDef)
{
    ...
    for (currId = 0; currId < MAX_MON_MOVES; currId++)
    {
        ...
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            ...
            {
                noOfHits[i] = GetNoOfHitsToKOBattler(battlerAtk, battlerDef, i, AI_ATTACKING_ON_FIELD, CONSIDER_ENDURE);
...
static s32 AI_CheckViability(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    {
        if (GetNoOfHitsToKOBattler(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex, AI_ATTACKING_ON_FIELD, DONT_CONSIDER_ENDURE) == 0)
...
static s32 AI_PreferStrongestMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (GetNoOfHitsToKOBattler(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex, AI_ATTACKING_ON_FIELD, CONSIDER_ENDURE) == 1)
        ADJUST_SCORE(BEST_EFFECT);
    else if (GetNoOfHitsToKOBattler(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex, AI_ATTACKING_ON_FIELD, CONSIDER_ENDURE) == 2)
        ADJUST_SCORE(DECENT_EFFECT);
